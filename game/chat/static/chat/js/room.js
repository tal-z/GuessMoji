...
let userEmoji = JSON.parse(document.getElementById("user-emoji").textContent);

const domain = JSON.parse(document.getElementById("domain").textContent);

let leader = JSON.parse(document.getElementById("start-leader").textContent);
...
const copyRoomButton = document.querySelector("#copy-button");

const endRoundButton = document.querySelector("#end-round-button");

const newPromptButton = document.querySelector("#new-prompt-button");

newPromptButton.onclick = async function newPrompt() {
var call = $.ajax({
  dataType: "json",
...
if (Object.hasOwn(data, "message")) {
  chatLog.innerHTML += formatChatMessage(
    data.user_emoji,
    data.username,
    data.message,
    data.chat_mode,
...
let now = Date.now();
for (const [key, [timestamp, aliasEmoji]] of Object.entries(
  chatMembersText
)) {
...
for (const aliasKey in chatMembersText) {
  let [timestamp, aliasEmoji] = chatMembersText[aliasKey];
...
newRoundRequests.innerText =
  Math.floor(numMembers / 2) + 1 - numUpdateRequests;

// update emoji clue
if (Object.hasOwn(data, "update_emoji_clue") && userName != data.leader) {
...
if (Object.hasOwn(data, "start_next_round")) {
  leader = data["leader"];
  let nextRound = data["start_next_round"];
...
emojiClueInput.oninput = broadcastEmojiClueChange;

endRoundButton.onclick = function broadcastEndOfRoundVote() {
chatSocket.send(
  JSON.stringify({
...
const emojiInputIsOpen =
  emojiCollapse.attributes["aria-expanded"].value === "true";
const isLeader = userName === leader;
const shouldToggleEmojiInput =
  (!isLeader && emojiInputIsOpen) || (isLeader && !emojiInputIsOpen);
...
if (userName === leader) {
  let lastPos = insertAtCursor(emojiClueInput, event.detail.unicode);
...
copyRoomButton.onclick = function copyLink() {
...
copyRoomName.onfocus = function expandLinkText() {
copyRoomName.value = `${domain}/chat/room/${roomName}`;
};
copyRoomName.onblur = function shortenLinkText() {
...
emojiClueInput.onkeydown = function preventKeyboardInput(event) {
console.log(event);
...
function formatChatMessage(userEmoji, userName, messageText, chatMode) {
...
let emojiArray = [...value.matchAll(regExpEmoji)];
let emojis = emojiArray.map((item) => item[0]).join("");
return emojis;
};
